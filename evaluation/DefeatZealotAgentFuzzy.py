...
class DefeatZealotAgentFuzzy(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        if not UBUNTU_DEACTIVATE_CHAT and self.first:
            ...
            greetingMsg += " When you are ready to go to the next explanation,"
            greetingMsg += " please say : continue!"
            self.env.send_chat_messages([greetingMsg])
            input("Press Enter to continue...")
            explainMsg = "Before we begin, let me explain you how I work."
            explainMsg += " I will choose to either run, attack or blink based"
            explainMsg += " on estimations (poor, average, good) of two variables:"
            explainMsg += " the life of the Stalkers and the distance between"
            explainMsg += " the Zealots and the Stalkers. As you would do,"
            explainMsg += " I will look at these informations and estimate"
            explainMsg += " which action is the best to take. Now let's begin !"
            self.env.send_chat_messages([explainMsg])
            input("Press Enter to continue...")
        player_relative = obs.observation.feature_screen.player_relative
